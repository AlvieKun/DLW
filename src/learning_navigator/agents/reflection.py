...
agent uses the LLM to polish the deterministic narrative into a more natural,
encouraging tone.  When not configured, the rule-based narrative is returned
as-is — no degradation.
"""
...
class ReflectionAgent(BaseAgent):
    ...
    async def handle(self, message: MessageEnvelope) -> AgentResponse:
        ...
        try:
            ...
            llm = get_llm_client()
            if llm.enabled:
                enhanced = await self._llm_enhance_narrative(
                    narrative, state, sections
                )
                if enhanced:
                    narrative = enhanced
                    llm_enhanced = True
                    confidence = min(1.0, confidence + 0.15)
                    log.info("reflection.llm_enhanced")
        except Exception as exc:
            log.warning("reflection.llm_fallback", error=str(exc))
        # ───────────────────────────────────────────────────────────
    ...
    # ── LLM hybrid enhancement ─────────────────────────────────────

    @staticmethod
    async def _llm_enhance_narrative(
        raw_narrative: str,
        state: LearnerState,
        sections: list[dict[str, str]],
    ) -> str | None:
        """Use Azure OpenAI to polish the deterministic narrative.

        Returns the enhanced text, or None if the LLM call failed
        (so the caller can fall back to the raw narrative).
        """
        from learning_navigator.llm import get_llm_client
        ...
        system_prompt = (
            "You are a warm, encouraging learning coach. "
            "You are given a structured learning progress report generated by an AI tutoring system. "
            "Rewrite it into a cohesive, natural-sounding narrative addressed directly to the learner. "
            "Keep ALL factual data (numbers, concept names, percentages) exactly as given. "
            "Do NOT invent new facts. Make it feel personal and motivating. "
            "Use 2nd person ('you'). Keep it concise — no more than 50% longer than the input. "
            "Return ONLY the rewritten narrative, no preamble."
        )

        learner_context = (
            f"Learner: {state.learner_id}, "
            f"avg mastery: {state.average_mastery():.0%}, "
            f"session count: {state.session_count}, "
            f"concepts tracked: {len(state.concepts)}"
        )

        user_prompt = (
            f"Context: {learner_context}\n\n"
            f"Raw report:\n{raw_narrative}"
        )
